...
export const bookNewAppointment = (appointmentData: Omit<Appointment, 'id' | 'totalPrice' | 'totalDuration'>): Appointment => {
    ...
    for (let i=0; i < numSlotsRequired; i++) {
        ...
        if (!availableSlots.includes(timeString)) {
             throw new Error(`Booking conflict. Not enough consecutive slots available for the selected services.`);
        }
    }
...
export const cancelAppointment = (details: { customerEmail: string, date: string, time: string }): Appointment => {
    const { customerEmail, date, time } = details;
    const appointmentIndex = appointmentsDB.findIndex(app => 
...
export const unblockSlot = (date: Date, time: string): AdminSettings['blockedSlots'] => {
    const dateKey = dateToKey(date);
    if (adminSettingsDB.blockedSlots[dateKey]) {
        adminSettingsDB.blockedSlots[dateKey] = adminSettingsDB.blockedSlots[dateKey].filter(t => t !== time);
    }
     return adminSettingsDB.blockedSlots;
}
