...
import { getServices, getAvailability, bookNewAppointment, cancelAppointment as dbCancelAppointment } from '../lib/database';
...
const checkAvailability: FunctionDeclaration = {
    ...
    parameters: {
        type: Type.OBJECT,
        properties: {
            date: { type: Type.STRING, description: 'The date to check for availability, in YYYY-MM-DD format.' },
        },
        required: ['date'],
    }
};
...
const bookAppointment: FunctionDeclaration = {
    ...
    parameters: {
        ...
        properties: {
            ...
            time: { type: Type.STRING, description: 'The start time of the appointment in HH:MM format.' },
        },
...
const cancelAppointment: FunctionDeclaration = {
    ...
    parameters: {
        type: Type.OBJECT,
        properties: {
            customerEmail: { type: Type.STRING, description: "The customer's email address used for the booking." },
            date: { type: Type.STRING, description: 'The date of the appointment to cancel, in YYYY-MM-DD format.' },
            time: { type: Type.STRING, description: 'The start time of the appointment to cancel, in HH:MM format.' },
        },
...
export const handleWhatsAppMessage = async (
    ...
    const servicesListString = allServices.map(s => `${s.name}: $${s.price} (${s.duration} mins)`).join('\n');
...
`;
    ...
    try {
        const response: GenerateContentResponse = await ai.models.generateContent({
            ...
            config: {
                systemInstruction,
                tools: [{ functionDeclarations: [getServicesList, checkAvailability, bookAppointment, cancelAppointment] }]
            }
        });
        ...
        if (response.functionCalls && response.functionCalls.length > 0) {
            ...
            if (name === 'cancelAppointment') {
                try {
                    const { customerEmail, date, time } = args;
