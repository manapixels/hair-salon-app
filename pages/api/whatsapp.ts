...
async function sendWhatsAppReply(to: string, text: string): Promise<void> {
    ...
    if (!phoneNumberId || !accessToken) {
        console.error("WhatsApp environment variables (WHATSAPP_PHONE_NUMBER_ID, WHATSAPP_ACCESS_TOKEN) are not set.");
        ...
        console.log(`SIMULATING sending WhatsApp reply to ${to}: "${text}" (because credentials are not set)`);
        return;
    }
...
export async function handlePost(requestBody: any) {
    ...
    if (message && message.type === 'text') {
        ...
        return { status: 200, body: { success: true, message: "Payload received but not a user text message." } };
    }
}