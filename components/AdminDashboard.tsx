...
const AdminDashboard: React.FC = () => {
  const { adminSettings, getAvailableSlots, blockTimeSlot, unblockTimeSlot, appointments, fetchAndSetAdminSettings, saveAdminSettings } = useBooking();
  ...
  const handleTimeSlotClick = async (time: string, isAvailable: boolean) => {
    // Optimistic update
    setTimeSlots(prev => prev.map(slot => slot.time === time ? {...slot, available: !isAvailable} : slot));
    try {
        if (isAvailable) {
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
        <div className="bg-gray-50 dark:bg-gray-700 p-4 rounded-lg">
            ...
            <div className="flex items-center space-x-4">
                <div>
                    <label htmlFor="openingTime" className="block text-sm font-medium text-gray-600 dark:text-gray-300">Opening Time</label>
                    <input type="time" id="openingTime" value={openingTime} onChange={e => setOpeningTime(e.target.value)} className="mt-1 p-2 border rounded-md bg-white dark:bg-gray-600 dark:border-gray-500" step="1800" />
                </div>
                <div>
                    <label htmlFor="closingTime" className="block text-sm font-medium text-gray-600 dark:text-gray-300">Closing Time</label>
                    <input type="time" id="closingTime" value={closingTime} onChange={e => setClosingTime(e.target.value)} className="mt-1 p-2 border rounded-md bg-white dark:bg-gray-600 dark:border-gray-500" step="1800" />
                </div>
                <button onClick={handleSettingsSave} className="self-end bg-indigo-600 text-white px-4 py-2 rounded-md hover:bg-indigo-700 transition-colors">Save</button>
            </div>
        </div>
         <div className="bg-gray-50 dark:bg-gray-700 p-4 rounded-lg">
             <h3 className="text-xl font-semibold mb-3">Appointments Today</h3>
             <p className="text-4xl font-bold text-indigo-600">{appointments.filter(a => new Date(a.date).toDateString() === new Date().toDateString()).length}</p>
        </div>
      </div>
      ...
      <div>
        ...
        {loading ? <p>Loading...</p> : (
            ...
            {timeSlots.map(({ time, available }) => {
                const isBooked = appointments.some(a => new Date(a.date).toDateString() === selectedDate.toDateString() && a.time === time);
                let buttonClass = '';
                if(isBooked) {
                    buttonClass = 'bg-red-500 text-white cursor-not-allowed';
                } else if (available) {
                    buttonClass = 'bg-green-100 dark:bg-green-800 text-green-800 dark:text-green-100 hover:bg-green-200 dark:hover:bg-green-700';
                } else {
                    buttonClass = 'bg-gray-300 dark:bg-gray-600 text-gray-500 dark:text-gray-400 hover:bg-gray-400 dark:hover:bg-gray-500';
                }
                ...
                return (
                    ...
                    {!isBooked && (available ? <span className="block text-xs mt-1">Available</span> : <span className="block text-xs mt-1">Blocked</span>)}
                    </button>
                )
            })}
            </div>
