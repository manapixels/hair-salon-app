...
const WhatsAppChat: React.FC = () => {
    const [messages, setMessages] = useState<WhatsAppMessage[]>([
        { id: '1', text: "Welcome to Luxe Cuts! How can I help you today? You can ask about our services, book, or cancel an appointment.", sender: 'bot' }
    ]);
    ...
    const handleSend = async () => {
        ...
        const loadingMessage: WhatsAppMessage = { id: 'loading', text: '', sender: 'bot', isLoading: true };
        ...
        try {
            ...
            const botMessage: WhatsAppMessage = { id: (Date.now() + 1).toString(), text: reply, sender: 'bot' };
            setMessages(prev => [...prev.filter(m => m.id !== 'loading'), botMessage]);

        } catch (error) {
            console.error("Error handling WhatsApp message:", error);
            const errorMessage: WhatsAppMessage = { id: (Date.now() + 1).toString(), text: "Sorry, I encountered an error. Please try again.", sender: 'bot' };
    ...
    const MessageBubble: React.FC<{ message: WhatsAppMessage }> = ({ message }) => {
        ...
        return (
            <div className={`flex ${isUser ? 'justify-end' : 'justify-start'}`}>
                <div className={`${isUser ? 'bg-green-600 text-white' : 'bg-white dark:bg-gray-700'} rounded-lg p-3 max-w-sm whitespace-pre-wrap`}>
                    {message.text}
                </div>
            </div>
        );
        ...
        <div className="flex flex-col h-[75vh] max-w-2xl mx-auto bg-gray-200 dark:bg-gray-900 shadow-2xl rounded-lg overflow-hidden">
            ...
            <div className="flex-1 p-4 space-y-4 overflow-y-auto bg-gray-100 dark:bg-gray-800">
                <div className="text-center p-3 mb-2 bg-blue-100 dark:bg-blue-900/50 text-blue-800 dark:text-blue-200 rounded-lg text-sm shadow-sm">
                    <p><i className="fa-solid fa-circle-info mr-2"></i>This is a simulation of our AI assistant. You can test its capabilities here, or use the QR code in the "Book Online" tab to chat on your own WhatsApp!</p>
                </div>
                {messages.map((msg) => (
            ...
            <div className="p-4 bg-white dark:bg-gray-700 flex items-center">
                ...
                />
                <button onClick={handleSend} className="ml-3 bg-green-600 text-white p-3 rounded-full hover:bg-green-700">
