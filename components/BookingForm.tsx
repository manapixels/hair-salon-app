...
const ServiceSelector: React.FC<{ selectedServices: Service[], onServiceToggle: (service: Service) => void }> = ({ selectedServices, onServiceToggle }) => (
    <div>
        <h2 className="text-2xl font-semibold mb-4 text-gray-800 dark:text-gray-200">1. Select Services</h2>
...
const DateTimePicker: React.FC<{
    ...
    return (
        <div className="mt-8">
            <h2 className="text-2xl font-semibold mb-4 text-gray-800 dark:text-gray-200">2. Select Date & Time</h2>
            <input
                type="date"
            ...
            />
            {loading ? <div className="text-center p-4">Loading time slots...</div> : (
                <div className="grid grid-cols-4 sm:grid-cols-6 md:grid-cols-8 gap-3">
                    {timeSlots.length > 0 ? timeSlots.map(({ time, available }) => (
                        <button
                            key={time}
                            disabled={!available}
                    ...
                    )) : <p className="col-span-full text-gray-500">No available slots for this day. Please try another date.</p>}
                </div>
            )}
             <p className="text-sm text-gray-500 mt-2">Total duration: {totalDuration} minutes. The system will ensure enough consecutive slots are free.</p>
        </div>
    );
};
...
const ConfirmationForm: React.FC<{
    ...
    return (
        <div className="mt-8">
            <h2 className="text-2xl font-semibold mb-4 text-gray-800 dark:text-gray-200">3. Confirm Your Booking</h2>
            <form onSubmit={handleSubmit} className="space-y-4 max-w-md">
                <div>
                    <label htmlFor="name" className="block text-sm font-medium text-gray-700 dark:text-gray-300">Full Name</label>
                    <input type="text" id="name" value={name} onChange={e => setName(e.target.value)} disabled={!!user} className="mt-1 block w-full px-3 py-2 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 disabled:bg-gray-100 dark:disabled:bg-gray-700" required />
                </div>
                 <div>
                    <label htmlFor="email" className="block text-sm font-medium text-gray-700 dark:text-gray-300">Email Address</label>
                    <input type="email" id="email" value={email} onChange={e => setEmail(e.target.value)} disabled={!!user} className="mt-1 block w-full px-3 py-2 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 disabled:bg-gray-100 dark:disabled:bg-gray-700" required />
                </div>
    ...
    return (
        <div className="p-6 bg-white dark:bg-gray-900/50 rounded-lg shadow-lg sticky top-8">
            <div className="flex justify-between items-center mb-4 border-b pb-3 dark:border-gray-700">
                ...
                <button onClick={onClear} className="text-sm text-red-500 hover:underline">&times; Clear All</button>
            </div>
            {selectedServices.length === 0 ? <p className="text-gray-500">Select services to get started.</p> : (
            <>
                ...
                <div className="border-t pt-3 dark:border-gray-700 space-y-2">
                    ...
                    {selectedTime && <p className="text-sm font-bold text-indigo-500">Time: {selectedTime}</p>}
                </div>
    ...
    if (bookingConfirmed) {
        return (
             <div className="text-center p-8 bg-white dark:bg-gray-800 rounded-lg shadow-lg max-w-lg mx-auto">
                ...
                <h2 className="mt-4 text-3xl font-bold text-gray-900 dark:text-white">Booking Confirmed!</h2>
                <p className="mt-2 text-gray-600 dark:text-gray-300">Thank you, {bookingConfirmed.customerName}.</p>
                <div className="mt-6 text-left bg-gray-50 dark:bg-gray-700 p-4 rounded-md space-y-2">
                   <p><strong>Services:</strong> {bookingConfirmed.services.map(s => s.name).join(', ')}</p>
                   <p><strong>Date:</strong> {new Date(bookingConfirmed.date).toLocaleDateString()}</p>
                   <p><strong>Time:</strong> {bookingConfirmed.time}</p>
                   <p><strong>Total:</strong> ${bookingConfirmed.totalPrice}</p>
                </div>
                <p className="mt-4 text-sm text-gray-500">A confirmation has been sent to {bookingConfirmed.customerEmail}.</p>
                 <button onClick={handleReset} className="mt-6 bg-indigo-600 text-white px-6 py-2 rounded-md hover:bg-indigo-700 transition-colors">
